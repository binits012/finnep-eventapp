...
export default function VenuesPage() {
  ...
  const countries = useMemo(() => {
    const countryList = [...new Set(venues.map(venue => venue.country).filter(Boolean))];
    return countryList;
  }, [venues]);
  ...
  if (venuesLoading || translationsLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto"></div>
          <p className="mt-4 text-gray-600 dark:text-gray-400">{t('venues.loading') || 'Loading venues...'}</p>
        </div>
      </div>
    );
  }
  ...
  if (venuesError) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 dark:text-red-400">{t('venues.error') || 'Error loading venues:'} {venuesError}</p>
        </div>
      </div>
    );
  }
          ...
          <div className="max-w-4xl mx-auto text-center text-white">
            ...
            >
              ...
              <p className="text-lg md:text-xl mb-8 text-blue-100 max-w-2xl mx-auto leading-relaxed">
                {t('venues.exploreVenues') || 'Explore stunning venues that host amazing events'}
              </p>
            </motion.div>
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-3 mb-3">
              ...
              <span className="text-sm font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">
                {t('venues.venuesFound', { count: filteredVenues.length }) || `${filteredVenues.length} Venues Found`}
              </span>
            ...
            <h2 className="text-3xl md:text-4xl font-bold mb-3 text-slate-800 dark:text-white">
              {t('venues.stunningVenues') || 'Stunning Venues'}
            </h2>
            <p className="text-base text-gray-600 dark:text-gray-400 max-w-xl mx-auto">
              {t('venues.venueDescription') || 'Each venue carefully selected for unforgettable experiences.'}
            </p>
          </motion.div>
              ...
              <div className="flex items-center gap-4">
                ...
                {selectedCountry && (
                  <div className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-full">
                    <span className="text-sm text-slate-700 dark:text-slate-300">{t('venues.filteredBy') || 'Filtered by:'}</span>
          ...
          {filteredVenues.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">{t('venues.noVenuesFound') || 'No Venues Found'}</h3>
              <p className="text-gray-600 dark:text-gray-400 text-lg max-w-md mx-auto">
                {t('venues.noVenuesMessage') || 'We couldn\'t find any venues matching your criteria. Try adjusting your search or filters.'}
              </p>
            </motion.div>
          ) : (
                    ...
                    <div className="relative h-64 overflow-hidden">
                      ...
                      <div className="absolute top-4 right-4">
                        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm text-gray-900 dark:text-white px-3 py-1.5 rounded-full text-sm font-semibold shadow-lg">
                          {venue.eventsCount} {venue.eventsCount === 1 ? t('venues.event') || 'Event' : t('venues.events') || 'Events'}
                        </div>
                      </div>
                    ...
                    <div className="p-6">
                      ...
                      {venue.upcomingEvents && venue.upcomingEvents.length > 0 && (
                        <div className="mb-6 p-4 bg-gradient-to-r from-slate-50 to-gray-50 dark:from-slate-700 dark:to-gray-600 rounded-xl">
                          <h4 className="text-sm font-bold mb-3 text-gray-900 dark:text-white flex items-center">
                            ...
                            {t('venues.upcomingEvents') || 'Upcoming Events'}
                          </h4>
                          <div className="space-y-2">
                            ...
                            {venue.upcomingEvents.length > 2 && (
                              <p className="text-xs text-slate-600 dark:text-slate-400 font-medium">
                                +{venue.upcomingEvents.length - 2} {t('venues.moreEvents') || 'more events'}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        return (
                          <div className="mb-6 p-4 bg-gradient-to-r from-slate-50 to-gray-50 dark:from-slate-700 dark:to-gray-600 rounded-xl">
                            <h4 className="text-sm font-bold mb-3 text-gray-900 dark:text-white flex items-center">
                              ...
                              {t('venues.connectWithVenue') || 'Connect with Venue'}
                              <span className="ml-2 px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-full text-xs font-medium">
                                {linkCount} {t('venues.link') || 'link'}{linkCount !== 1 ? 's' : ''}
                              </span>
                            </h4>
                          <div className="flex flex-wrap gap-3">
                            {/* Website Link */}
                      ...
                      <Link href={`/events?venue=${encodeURIComponent(venue.venueName)}`}>
                        <span className="group/btn inline-flex items-center justify-center w-full py-3 px-6 bg-gradient-to-r from-slate-600 to-slate-800 text-white rounded-xl hover:from-slate-700 hover:to-slate-900 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                          ...
                          {t('venues.viewEventsAtVenue') || 'View Events at this Venue'}
                        </span>
                      </Link>
                    </div>
                  </div>
